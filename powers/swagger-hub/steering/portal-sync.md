...
```
sections = mcp__smartbear-mcp__swagger_list_portal_product_sections(
...
```
doc = mcp__smartbear-mcp__swagger_get_document(documentId)
mcp__smartbear-mcp__swagger_update_document(documentId, newContent)
```
Content can be HTML or Markdown. Confirm the content type with the user.

### 5. Preview Before Publishing
```
mcp__smartbear-mcp__swagger_publish_portal_product(portalId, productId, preview: true)
```
